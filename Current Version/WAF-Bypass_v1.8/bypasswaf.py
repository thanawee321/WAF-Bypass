...
class INFO:
    ...
    default_wordlists = "wordlists.txt"
    updated_wordlists = "wordlists.txt"
...
def get_domain_historical_ip_address(domain):
    ...
    try:
        """service = Service("/usr/bin/chromedriver")  # ใส่ path ให้ตรงกับระบบ
...
############FIND_SUBDOMAIN_SSL###############################
def find_subdomains_with_ssl_analysis(domain,wordlists_path_updated):
    ...
    def check_subdomain(subdomain):
        ...
        try:
            response = requests.get(subdomain_url, timeout=(2,2),verify=True)
            if response.status_code == 200:
                ...
                print(f"{Fore.RED}[+] Subdomain not Found {Fore.CYAN}\u2514\u27A4{Fore.WHITE} {subdomain_url}{Fore.RESET}")
    ...
    print(f"\n{Fore.YELLOW}[!] Starting threads...{Fore.RESET}")
        ...
        if waf_status:
            ...
            if args.wordlists:
                ...
                if not os.path.exists(custom_path_wordlists):
                    ...
                    find_subdomains_with_ssl_analysis(domain,wordlists_path_updated)
                
                find_subdomains_with_ssl_analysis(domain,custom_path_wordlists)
